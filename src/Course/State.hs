...
findM f (h :. t) = f h >>= fnd where
  fnd a = if a then pure (Full h) else findM f t

-- findM f as = (fst <$>) <$> find snd <$> (sequence $ tupled <$> as)
--   where
--     tupled a = (\b -> (a, b)) <$> f a
...
firstRepeat = evalStateSet <$> (findM stateSetMember)
...
distinct = evalStateSet <$> (filtering stateSetNotMember)



evalStateSet :: State (S.Set s) a -> a
evalStateSet = flip eval $ S.empty

stateSetMember :: Ord a => a -> State (S.Set a) Bool
stateSetMember = rollStateSet S.member

stateSetNotMember :: Ord a => a -> State (S.Set a) Bool
stateSetNotMember = (not <$>) . stateSetMember

rollStateSet ::  Ord a => (a -> S.Set a -> Bool) -> a -> State (S.Set a) Bool
rollStateSet f a = do s <- get
                      if f a s
                         then return True
                         else do put $ S.insert a s
                                 return False
...
next :: Show a => a -> Integer
next x = toInteger $ sum $ join (*) . digitToInt <$> show' x
